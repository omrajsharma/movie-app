      ...
      <div v-if="searchInput !== ''" id="movie-grid" class="movies-grid">
        <div class="movie" v-for="(movie,index) in searchedMovies" :key="index" >
          ...
          <NuxtLink
              ...
              :to="{ name: 'movie-movieid', params: { id: movie.id } }"
            >
              Get More Info
            </NuxtLink>
        </div>
  ...
  methods: {
    async getMovies(){
      const data = axios.get(
        'https://api.themoviedb.org/3/movie/now_playing?api_key=a9e31be55f1bfc71ac57b36ca850a05d&language=en-US&page=1'
      );
    ...
    async searchMovies() {
      const data = axios.get(`
        https://api.themoviedb.org/3/search/movie?api_key=a9e31be55f1bfc71ac57b36ca850a05d&language=en-US&query=${this.searchInput}&page=1&include_adult=false`
      )
      const result = await data;
      result.data.results.forEach((movie) => {
